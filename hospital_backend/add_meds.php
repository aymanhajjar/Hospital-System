...
if ($base64_url_signature === $signature) {
    ...
    $service = $mysqli->prepare('insert into users_has_medications (user_id, medication_id, quantity) values(?,?,?)');
    $service->bind_param('iis', $decoded_payload->id, $med, $quantity);
    $service->execute();
    $service->store_result();
    $response['status'] = 'added med';
